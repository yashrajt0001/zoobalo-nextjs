...
const ExtraTiffinCard: FC<userInterface> = ({ data }) => {
  ...
  const handleSelection = async () => {
    try {
      await axios.patch(
        `${process.env.NEXT_PUBLIC_HOST}/kitchenHead/selection`,
        {
  ...
  const handleDone = async () => {
    try {
      await axios.patch(
        `${process.env.NEXT_PUBLIC_HOST}/extraTiffin/done`,
        {
          review: reviewText,
          extraTiffinId: data.id,
        },
  ...
  return (
    ...
    >
      ...
      <div className="flex w-full flex-row justify-between">
        {showCompleted ? (
          <div>
            <h1 className="text-base">{data?.review}</h1>
          </div>
        ) : (
...
export default ExtraTiffinCard;
